...
function getAdbPath(): string {
  // En développement, utiliser le chemin local vers platform-tools
  if (is.dev) {
    // Note: Le chemin est relatif à la racine du projet Electron, qui est DimiCall/
    return join(app.getAppPath(), 'platform-tools-latest-windows (4)', 'platform-tools', 'adb.exe')
  }

  // En production, utiliser le chemin dans les ressources de l'application
  // electron-builder copie les extraResources ici.
